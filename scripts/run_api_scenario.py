...
def non_empty_neighbor_directions(state: Dict, row: int, col: int) -> List[int]:
    directions = []
    for direction in range(4):
        next_row, next_col, _ = neighbor(row, col, direction)
        if in_bounds(next_row, next_col) and cell_type(state, next_row, next_col) != "empty":
...
def find_adjacent_output(state: Dict, row: int, col: int, prev_cell: Optional[Tuple[int, int]]) -> int:
    candidates = []
    for direction in non_empty_neighbor_directions(state, row, col):
        next_row, next_col, _ = neighbor(row, col, direction)
    ...
    raise ApiError(f"Ambiguous forward neighbor from cell ({row}, {col}). Expected one, got {len(candidates)}.")
...
def choose_rotary_exit(state: Dict, row: int, col: int, prev_cell: Optional[Tuple[int, int]], rng: random.Random) -> int:
    ...
    if prev_cell is not None:
        dr = prev_cell[0] - row
        dc = prev_cell[1] - col
        forbidden_direction = DELTA_TO_DIR.get((dr, dc))
